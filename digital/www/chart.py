...
def get_context(context):
    ...
    daily_totals = defaultdict(float)
    for p in payments:
        # Ensure payment_time_and_data is a datetime
        payment_date = (
            p["payment_time_and_data"].strftime("%Y-%m-%d")
            if isinstance(p["payment_time_and_data"], datetime)
            else str(p["payment_time_and_data"])
        )
        daily_totals[payment_date] += float(p.get("product_price") or 0)
