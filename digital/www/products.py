...
def get_context(context):
    ...
    context.categories = [
        row.category for row in frappe.get_all(
            "Digital Product",
            filters={"docstatus": 1},
            fields=["category"],
            distinct=True
    ...
    creator = frappe.get_all("Creator Profile", fields=["name", "logo"], limit=1)
    context.creator_logo = creator[0]["logo"] if creator else None
