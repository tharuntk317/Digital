...
def execute(filters=None):
    ...
    products = list({d['product'] for d in chart_data if d['product']})
    days = sorted([d['day'] for d in chart_data if d['day']]) 
    ...
    for product in products:
        ...
        for day in days:
            found = next(
                (d['count'] for d in chart_data if d['day'] == day and d['product'] == product),
                0
            )
